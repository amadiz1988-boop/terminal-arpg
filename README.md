...
## Canonical reconstruction from GitHub (V6)

Clone `amadiz1988-boop/terminal-arpg` branch `main`. With authenticated access
to the private asset repository, run:

```powershell
python -B scripts/fetch-web-runtime-private-release.py --checkout . --destination C:\private-assets\web-v1.1.0 --materialize
node scripts/test-canonical-web-offline.mjs
```

Use a new private destination outside the clone. Credentials are read from the
Git credential manager or GH_TOKEN/GITHUB_TOKEN and are never saved by the
fetcher. Asset verification precedes extraction and materialization. The ignored
runtime files remain outside Git. For Native, clone the PRIVATE
`amadiz1988-boop/ghost-island-rathena` main and follow its
`CANONICAL_RECONSTRUCTION_V6.md`. These are offline source/build procedures.
