  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if ([4,5,6,7,8].includes(s.schemaVersion)) {
          setClassChosen(s.classChosen); setClassId(s.classId); setSecondJobId(s.secondJobId); setAscendancyNodes(s.ascendancyNodes); setTalentBoard(s.talentBoard); setTalents((s.talents??[]).filter((id:string)=>TALENTS.some(node=>node.id===id)));
          ...
          if(s.schemaVersion===8&&s.combat&&['search','travel','combat'].includes(s.combat.phase)){setCombat(s.combat);setProgress(Math.min(100,Math.round(s.combat.kills/s.combat.totalMonsters*100)));}
        }
      }
