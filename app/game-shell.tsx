...
import { resolveDefaultAttack, resolveStats } from '@/game/modifiers/resolve-stats';
...
import { contractFailureChance, generateMonsterPacks, generateMonsterPopulation, getRunStopReason, selectCampaignPacks, simulateMapCompletion } from '@/game/simulation/map';
...
export function GameShell() {
  ...
  const build = { ...equippedItems, level, skill: activeSkill, socketItem, socketSlot:gemHostSlot, supports: selectedSupports.map((id) => SUPPORTS[id]), supportSlots: linkCount, roStats, classId, secondJobId, ascendancyNodes };
  const resolved = resolveStats(build);
  const defaultAttack = resolveDefaultAttack(build);
  const dps = resolved.dps;
  const statusAttack = activeSkill.tags.includes('bow') ? roRangedAttack(roStats,level) : roMeleeAttack(roStats,level);
  const attackRemainder = Math.max(0, defaultAttack.hitDamage - statusAttack);
  const displayedAspd = Math.floor(roAspdFromAttacksPerSecond(defaultAttack.attacksPerSecond) * 10) / 10;
  ...
  const encounterPacks=inCampaign?selectCampaignPacks(mapPacks):mapPacks;
  const encounterPopulation={normal:encounterPacks.reduce((sum,pack)=>sum+pack.normal,0),magic:encounterPacks.reduce((sum,pack)=>sum+pack.magic,0),rare:encounterPacks.reduce((sum,pack)=>sum+pack.rare,0),special:encounterPacks.reduce((sum,pack)=>sum+pack.special,0)};
  const mapTerrain=generateTerrain(seedRef.current+1);
  const mapRoster=generateMonsterRoster(seedRef.current+1,tier,encounterPacks,inCampaign?level:undefined);
  ...
  const trailCells=new Set(combat?.trail??[]);
  const playerHurt=Boolean(combat?.events.some(event=>event.kind==='DAMAGE'));
  const attackVisible=combat?.attackFrom!==undefined&&combat.attackTo!==undefined;
  const attackFrom=attackVisible?{x:((combat!.attackFrom!%MAP_WIDTH)+.5)/MAP_WIDTH*100,y:(Math.floor(combat!.attackFrom!/MAP_WIDTH)+.5)/(MAP_SIZE/MAP_WIDTH)*100}:undefined;
  const attackTo=attackVisible?{x:((combat!.attackTo!%MAP_WIDTH)+.5)/MAP_WIDTH*100,y:(Math.floor(combat!.attackTo!/MAP_WIDTH)+.5)/(MAP_SIZE/MAP_WIDTH)*100}:undefined;
    ...
    try {
      ...
      if (raw) {
        ...
        if ([4,5,6,7,8,9].includes(s.schemaVersion)) {
          setClassChosen(s.classChosen); setClassId(s.classId); setSecondJobId(s.secondJobId); setAscendancyNodes(s.ascendancyNodes); setRoStats(repairRoStatsForLevel(s.roStats,1+Math.floor((s.xp??0)/100)));
  ...
  const equipItem = (item:Item) => {
    ...
    pushLog('POWER',`換上 ${item.name} · DPS ${dps.toLocaleString()} → ${next.dps.toLocaleString()} · 生命 ${resolved.life.toLocaleString()} → ${next.life.toLocaleString()} · 護甲 ${resolved.armor.toLocaleString()} → ${next.armor.toLocaleString()}`);
  };
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(()=>setCombat(current=>{
      if(!current)return current;const rewardPlan=current.rewardPlan??simulateMapCompletion(seedRef.current+1,tier,inCampaign?'full-clear':policy,build,contractId,'arsenal',true);const next=stepCombat(current,{tier,level,build,packs:encounterPacks,seed:seedRef.current+1,tickMs:125,areaLevel:inCampaign?level:undefined});next.rewardPlan=rewardPlan;
      if(next.kills>current.kills){let rewardProgress=current.rewardProgress;const ranks=next.killRanks?.length?next.killRanks:next.lastKillRank?[next.lastKillRank]:[];ranks.forEach((rank,index)=>{const reward=progressMapRewards(rewardPlan,rewardProgress,current.kills+index+1,next.totalMonsters,rank);rewardProgress=reward.progress;deliverLiveRewards(rewardPlan,reward.delta);});next.rewardProgress=rewardProgress;}
      setProgress(Math.min(100,Math.round(next.kills/next.totalMonsters*100)));
      const visible=combatDetail==='full'?next.events:next.events.filter(event=>event.kind==='MOVE'?next.playerPosition%3===0:!['HIT','BASIC'].includes(event.kind)||next.action%4===0||next.targetLife===0);
  ...
  },[running,combat,tier,level,dps,policy,skillId,runs,goal,mode,maps,contractId,combatDetail,secondJobId,items.length,salvageMode,roStats]);

  useEffect(()=>{logRef.current?.scrollTo({top:logRef.current.scrollHeight,behavior:'smooth'});},[logs]);
  useEffect(()=>{const viewport=mapViewportRef.current;if(!viewport)return;const position=combat?.playerPosition??mapTerrain.start,x=(position%MAP_WIDTH+.5)/MAP_WIDTH,y=(Math.floor(position/MAP_WIDTH)+.5)/(MAP_SIZE/MAP_WIDTH);viewport.scrollTo({left:Math.max(0,viewport.scrollWidth*x-viewport.clientWidth/2),top:Math.max(0,viewport.scrollHeight*y-viewport.clientHeight/2),behavior:'smooth'});},[combat?.playerPosition,mapZoom]);
  ...
  const start=()=>{
    ...
    if(inCampaign){setRuns(0);setProgress(0);const next=createCombatState(1,level,build,encounterPacks,seedRef.current+1,level);setCombat(next);setRunning(true);pushLogs([['QUEST',`接受 ${operation.title} · ${operation.lesson}`],...next.events.map(event=>[event.kind,event.text] as [string,string])]);return;}
  ...
  const buyStat=(id:RoStatId)=>{const cost=nextStatCost(roStats[id]);if(roStats[id]>=99||statusPoints<cost)return;setRoStats(value=>({...value,[id]:value[id]+1}));const definition=RO_STAT_DEFINITIONS.find(stat=>stat.id===id);pushLog('STAT',`${definition?.name} ${roStats[id]} → ${roStats[id]+1} · 消耗 ${cost} 點`);};
  const resetStats=()=>{if(spentStatusPoints(roStats)===0)return;setRoStats({...DEFAULT_RO_STATS});pushLog('STAT','全部能力重置為 1 · 已返還所有能力點');};
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="scanlines" aria-hidden="true" />
    <header className="border-b border-border/80 bg-card/80"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 sm:px-6"><div className="flex items-center gap-3"><div className="brand-mark"><CircleDot className="size-5"/></div><div><div className="flex gap-2"><h1 className="font-mono text-sm font-bold tracking-[.16em] text-primary">TERMINAL ARPG</h1><span className="rounded border border-primary/30 px-1.5 font-mono text-[9px] text-primary">ALPHA 0.18</span></div><p className="text-[11px] text-muted-foreground">RO 職業 × 技能寶石 × 自動遠征</p></div></div><div className="text-right font-mono text-xs"><b className="text-primary">Lv.{level} · {classChosen?(secondJobId?SECOND_JOBS[secondJobId].name:CLASSES[classId].name):'未選職'}</b><small className="block text-muted-foreground">DPS {dps.toLocaleString()}</small></div></div></header>
    ...
    {classChosen&&<><div className="mx-auto grid max-w-[1500px] gap-4 px-4 py-4 sm:px-6 xl:grid-cols-[300px_minmax(0,1fr)_340px]">
      ...
      <section className="min-w-0 space-y-4">
        ...
        {bestUpgrade&&<div className="upgrade-alert"><div><small>剛取得可用升級</small><b>{bestUpgrade.item.name}</b><span>{SLOT_LABELS[bestUpgrade.item.slot]} · DPS {bestUpgrade.evaluation.dpsDelta>=0?'+':''}{bestUpgrade.evaluation.dpsDelta}% · 生存 {bestUpgrade.evaluation.survivalDelta>=0?'+':''}{bestUpgrade.evaluation.survivalDelta}%</span></div><button onClick={()=>equipItem(bestUpgrade.item)}>立即換裝</button></div>}
        <div className="map-frame">
          <div className="map-population"><b>{inCampaign?'新手區域':'本圖'} {encounterPopulation.normal+encounterPopulation.magic+encounterPopulation.rare+encounterPopulation.special+1} 隻</b><span>普通 {encounterPopulation.normal}</span><span className="magic-text">魔法 {encounterPopulation.magic}</span><span className="rare-text">稀有 {encounterPopulation.rare}</span><span className="special-text">特殊 {encounterPopulation.special}</span><span>首領 1</span><div className="map-zoom"><button aria-label="縮小地圖" disabled={mapZoom<=1} onClick={()=>setMapZoom(value=>Math.max(1,value-.5))}>−</button><em>{Math.round(mapZoom*100)}%</em><button aria-label="放大地圖" disabled={mapZoom>=2.5} onClick={()=>setMapZoom(value=>Math.min(2.5,value+.5))}>＋</button></div></div>
          <div ref={mapViewportRef} className="map-viewport"><div className="field-map" style={{width:`${mapZoom*100}%`}} aria-label="RO 式俯視迷霧地圖">
            {Array.from({length:MAP_SIZE},(_,i)=>{const visible=visibleCells.has(i),walkable=walkableCells.has(i),player=i===(combat?.playerPosition??mapTerrain.start),boss=i===mapTerrain.boss,occupants=livingByCell.get(i),count=occupants?.count??0,trail=trailCells.has(i);const title=count?`此格有 ${count} 隻未擊殺怪物`:boss?'首領房':walkable?'可行走地形':'障礙地形';const base=!visible?'fog':player?'player':boss?'boss':!walkable?'void':count?`monster ${occupants?.rank??'normal'}`:'floor';return <i key={i} title={title} className={`${base}${trail&&visible?' trail':''}${player&&playerHurt?' hurt':''}`}>{!visible?'':player?'◆':boss?'B':count?count>9?'●':String(count):''}</i>})}
            {attackVisible&&attackFrom&&attackTo&&(activeSkill.tags.includes('projectile')||activeSkill.tags.includes('spell'))&&<svg className="combat-vector" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><line x1={attackFrom.x} y1={attackFrom.y} x2={attackTo.x} y2={attackTo.y}/><circle cx={attackTo.x} cy={attackTo.y} r="1.1"/></svg>}
            <span>◆ 玩家　青線 移動軌跡　光束 投射物　閃紅 受傷</span>
          </div></div>
        </div>
      ...
      <aside className="space-y-4">
        ...
        <Panel title="人物能力與屬性狀態欄" icon={TrendingUp}><div className="ro-sheet"><div className="ro-primary">{RO_STAT_DEFINITIONS.map(stat=>{const value=roStats[stat.id],cost=nextStatCost(value);return <div key={stat.id}><span><b>{stat.id.toUpperCase()}</b><strong>{value}<i>+0</i></strong></span><button aria-label={`提升 ${stat.name}`} disabled={value>=99||statusPoints<cost} onClick={()=>buyStat(stat.id)}>+<small>{value>=99?'MAX':cost}</small></button></div>})}</div><div className="ro-derived"><StatLine label="ATK" value={`${statusAttack} + ${attackRemainder}`}/><StatLine label="DEF" value={`${roPhysicalDefense(roStats,level)} + ${resolved.armor}`}/><StatLine label="MATK" value={`${formatOne(roAverageMagicAttack(roStats,level))} + 0`}/><StatLine label="MDEF" value={`${roMagicDefense(roStats,level)} + 0`}/><StatLine label="HIT" value={String(roHit(level,roStats))}/><StatLine label="FLEE" value={`${roFlee(level,roStats)} + ${formatOne(roPerfectDodge(roStats))}`}/><StatLine label="CRITICAL" value={`${formatOne(resolved.critChance)}%`}/><StatLine label="ASPD" value={formatOne(displayedAspd)}/><StatLine label="STATUS POINT" value={String(statusPoints)}/></div></div><div className="mt-3 flex items-center justify-between gap-3"><p className="text-[10px] text-muted-foreground">Lv.1 從六圍各 1、剩餘 0 點開始。面板隨配點與裝備即時計算。</p><button disabled={spentStatusPoints(roStats)===0} onClick={resetStats} className="stat-reset">全部重置</button></div><details className="mt-3 text-[10px] text-muted-foreground"><summary className="cursor-pointer text-primary">查看六圍效果與資料來源</summary><div className="mt-2 space-y-1">{RO_STAT_DEFINITIONS.map(stat=><p key={stat.id}><b>{stat.name}</b>：{stat.summary}</p>)}</div><p className="mt-2">仙境全書角色數值、iRO Renewal Cast Time、iRO Classic ASPD。LUK 不增加掉寶率。</p></details></Panel>
...
function StatLine({label,value}:{label:string;value:string}){return <div><span>{label}</span><b>{value}</b></div>}
function formatOne(value:number){return Number.isInteger(value)?String(value):value.toFixed(1)}
