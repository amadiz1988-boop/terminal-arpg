...
export function GameShell() {
  ...
  const drops = world?.events.filter((event) => event.type === 'drop').length ?? 0;
  const pickups = world?.events.filter((event) => event.type === 'pickup').length ?? 0;
