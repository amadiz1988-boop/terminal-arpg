...
import { FARMING_ROUTES } from '@/game/content/farming-routes';
import type { ContractId, FarmingRouteId, Item, MasteryId, Policy, RunMode as Mode, SkillId } from '@/game/core/types';
...
export function GameShell() {
  ...
  const [routeId, setRouteId] = useState<FarmingRouteId>('arsenal');
  const [huntProgress, setHuntProgress] = useState(0);
  const [pendingChoices, setPendingChoices] = useState<Item[]>([]);
    ...
    try {
      ...
      if (raw) {
        ...
        if (save.schemaVersion === 1) {
          ...
          setRouteId(save.routeId ?? 'arsenal'); setHuntProgress(save.huntProgress ?? 0); setPendingChoices(save.pendingChoices ?? []);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('terminal-arpg-save', JSON.stringify({ schemaVersion: 1, seed: seedRef.current, skillId, policy, mode, goal, tier, maps, campaignStep, acquiredSkills, selectedUnlock, currency, xp, items, equipped, materials, totalKills, salvageMode, contractId, masteries, routeId, huntProgress, pendingChoices }));
  }, [saveLoaded, running, skillId, policy, mode, goal, tier, maps, campaignStep, acquiredSkills, selectedUnlock, currency, xp, items, equipped, materials, totalKills, salvageMode, contractId, masteries, routeId, huntProgress, pendingChoices]);
  ...
  const finishMap = () => {
    ...
    seedRef.current += 1;
    const result = simulateMapCompletion(seedRef.current, tier, policy, { skill, weapon, armor, masteries }, contractId, routeId);
    ...
    if (result.success) {
      ...
      if (nextHunt >= 4) {
        if (routeId === 'foundry') {
          setMaterials((value) => ({ scrap: value.scrap + 8, essence: value.essence + 2, core: value.core + 1 }));
          pushLog('HUNT', '餘燼熔爐結算 · 廢料 +8 · 精華 +2 · 核心 +1');
        } else {
          const forcedSlot = routeId === 'arsenal' ? 'weapon' : undefined;
          setPendingChoices([0, 1, 2].map((offset) => generateItem(seedRef.current + 4040 + offset, tier * 18 + 20, forcedSlot)));
          pushLog('HUNT', `${FARMING_ROUTES[routeId].name}結算 · 三件戰利品等待選擇`);
        }
        setHuntProgress(0);
    ...
    const nextTier = tier;
    const availableNext = projectedMaps[tier];
    ...
    window.setTimeout(() => {
      ...
      setTier(nextTier); setProgress(0); runStart.current = Date.now(); pushLog('MAP', `投入下一張 T${nextTier}，該階庫存剩餘 ${Math.max(0, availableNext - 1)} 張。`);
    }, 650);
  };
  ...
  }, [running, dps, tier, policy, skill.name, runs, goal, mode, maps, huntProgress, routeId]);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <header className="border-b border-border/80 bg-card/80 backdrop-blur-xl"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 sm:px-6"><div className="flex items-center gap-3"><div className="brand-mark"><CircleDot className="size-5" /></div><div><div className="flex items-center gap-2"><h1 className="font-mono text-sm font-bold tracking-[.18em] text-primary">TERMINAL ARPG</h1><span className="rounded border border-primary/30 bg-primary/10 px-1.5 py-.5 font-mono text-[9px] text-primary">ALPHA 0.5</span></div><p className="text-[11px] text-muted-foreground">刷圖產業 · 三選一戰利品 · 30 分鐘測試閘門</p></div></div><div className="flex gap-4 font-mono text-xs"><span>Lv.{level}</span><span className="text-amber-300">◈ {currency}</span><span className="hidden text-muted-foreground sm:inline">廢料 {materials.scrap} · 精華 {materials.essence}</span><span className="hidden text-muted-foreground sm:inline">XP {xp % 100}/100</span></div></div></header>
    ...
    <div className="mx-auto grid max-w-[1500px] gap-4 px-4 py-4 sm:px-6 xl:grid-cols-[300px_minmax(0,1fr)_330px]">
      <aside className="space-y-4">
        ...
        {!inCampaign && <Panel title="刷圖產業" icon={Crosshair}><div className="space-y-2">{(Object.keys(FARMING_ROUTES) as FarmingRouteId[]).map((id)=>{const route=FARMING_ROUTES[id];return <button aria-label={`${route.name}：${route.description}`} key={id} disabled={running} onClick={()=>{setRouteId(id);setHuntProgress(0);pushLog('PLAN',`切換產業：${route.name}`);}} className={`setting-row ${routeId===id?'active':''}`}><span><b>{route.name}{skillId===route.favoredSkill?' · BUILD 契合':''}</b><small>{route.description}</small></span><i aria-hidden="true" /></button>;})}</div><div className="mt-3 flex items-center justify-between text-xs"><span>{FARMING_ROUTES[routeId].chase}</span><b className="text-primary">{huntProgress}/4</b></div><div className="progress-track mt-2"><div className="progress-fill" style={{width:`${huntProgress*25}%`}} /></div></Panel>}
      ...
      <aside className="space-y-4">
        {!inCampaign && pendingChoices.length > 0 && <Panel title="戰利品三選一" icon={Box}><p className="mb-2 text-[11px] text-muted-foreground">比較目前 Build 的實際提升，只能帶走一件。</p><div className="space-y-2">{pendingChoices.map((item)=>{const evaluation=evaluateItem(item,{skill,weapon,armor,masteries});const gain=Math.max(evaluation.dpsDelta,evaluation.survivalDelta,evaluation.clearDelta);return <button key={item.id} onClick={()=>{setItems((value)=>[item,...value].slice(0,24));setPendingChoices([]);pushLog('CLAIM',`選擇 ${item.name} · 最佳提升 ${gain>=0?'+':''}${gain}%`);}} className="inventory-item"><span className={`rarity rarity-${item.rarity.toLowerCase()}`}>{item.rarity}</span><span><b>{gain>=0?'+':''}{gain}% · {item.name}</b><small>{formatAffixes(item)}</small></span><em>選擇</em></button>;})}</div></Panel>}
