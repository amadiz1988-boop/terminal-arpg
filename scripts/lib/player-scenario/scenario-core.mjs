...
const optionNames = new Set([
  ...
  '--dry-run', '--execute', '--matrix', '--help', '--trace-id', '--support-session',
  '--support-reason', '--support-mode', '--support-ttl',
]);
...
export function parseCli(argv = []) {
  const options = {
    ...
    supportReason: process.env.SYNTHETIC_SUPPORT_REASON ?? 'Synthetic Player scenario acceptance',
    supportMode: process.env.SYNTHETIC_SUPPORT_MODE ?? 'PLAYER_ACTIONS',
    supportTtlMinutes: Number(process.env.SYNTHETIC_SUPPORT_TTL_MINUTES ?? 15),
    supportSessionCookie: process.env.SYNTHETIC_SUPPORT_SESSION_COOKIE ?? null,
    help: false,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--json' || arg === '--matrix' || arg === '--support-session') {
      if (arg === '--support-session') options.supportSession = true;
    ...
    if (arg === '--support-reason') options.supportReason = value;
    if (arg === '--support-mode') options.supportMode = value;
    if (arg === '--support-ttl') options.supportTtlMinutes = Number(value);
  }
  ...
  if (options.supportSession && (!options.charId || !['OBSERVE_ONLY', 'PLAYER_ACTIONS'].includes(options.supportMode)))
    throw new Error('--support-session requires --char and a valid --support-mode');
  if (options.supportSession && (!Number.isSafeInteger(options.supportTtlMinutes) || options.supportTtlMinutes < 1 || options.supportTtlMinutes > 60))
    throw new Error('--support-ttl must be between 1 and 60 minutes');
  return options;
}
...
export function helpText() {
  return [
    ...
    '         --support-session --support-reason <text> --support-mode <mode> --support-ttl <minutes>',
  ].join('\n');
}
