...
try {
  ...
  socket.onmessage = (event) => {
    ...
    if (message.method === 'Network.loadingFailed') errors.push(`${message.params?.errorText} ${message.params?.blockedReason ?? ''}`);
    if (message.id && pending.has(message.id)) { pending.get(message.id)(message); pending.delete(message.id); }
  };
  await new Promise((resolve) => { socket.onopen = resolve; });
  const call = (method, params = {}) => new Promise((resolve) => { const id = ++requestId; pending.set(id, resolve); socket.send(JSON.stringify({ id, method, params })); });
  ...
  const report = await evaluate("({title:document.title,canvas:document.querySelector('.field-canvas').width,attack:document.querySelectorAll('.event-player_hit').length,death:document.querySelectorAll('.event-death').length,pickup:document.querySelectorAll('.event-pickup').length,baseExp:document.body.innerText.match(/人物經驗 ([0-9,]+)/)?.[1],jobExp:document.body.innerText.match(/職業經驗 ([0-9,]+)/)?.[1],mapInfo:document.body.innerText.includes('掉落物')&&document.body.innerText.includes('傑勒比結晶')&&document.body.innerText.includes('水 1'),pickedItems:document.body.innerText.includes('拾取物品')&&document.body.innerText.includes('傑勒比結晶'),summaryNoRawCounts:!document.body.innerText.includes('掉落 1')&&!document.body.innerText.includes('拾取 1'),englishItem:/\b(Jellopy|Sticky Mucus|Fly Wing|Poring Card)\b/.test(document.body.innerText),qaControlsAbsent:!document.body.innerText.includes('四倍測試')&&!document.body.innerText.includes('重新開始')&&!document.body.innerText.includes('4x'),overflow:document.documentElement.scrollWidth>innerWidth})");
  report.paused = beforePause === afterPause;
  report.errors = errors;
  report.pass = report.canvas === 720 && report.attack >= 3 && report.death >= 1 && report.pickup >= 1 && Number(report.baseExp?.replaceAll(',', '')) > 0 && Number(report.jobExp?.replaceAll(',', '')) > 0 && report.mapInfo && report.pickedItems && report.summaryNoRawCounts && !report.englishItem && report.qaControlsAbsent && report.paused && !report.overflow && errors.length === 0;
