...
try {
  ...
  const observeCombat = async (attempts = 3000) => {
    ...
    for (let attempt = 0; attempt < attempts && (!sawProjectile||!sawHurt); attempt += 1) {
      try { const pulse=await evaluate("({projectile:Boolean(document.querySelector('.combat-vector')),hurt:Boolean(document.querySelector('.field-map .player-marker.hurt'))})");sawProjectile ||= pulse.projectile;sawHurt ||= pulse.hurt; } catch { /* Animation frame replaced during observation. */ }
