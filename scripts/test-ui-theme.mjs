...
function boot(stored, { imageFails = () => false, storage: shared } = {}) {
  ...
  return { api: window.GhostIslandUiTheme, root, style, storage, probes };
}
...
test('every visit rotates through all images unless the player pins one', () => {
  ...
  assert.equal(boot(null, { storage }).root.dataset.uiThemeVariant, 'sylphie-05');
  assert.equal(boot(null, { storage }).root.dataset.uiThemeVariant, 'sylphie-05');
  boot(null, { storage }).api.selectVariant('heroine-sylphie', 'auto');
  assert.equal(boot(null, { storage }).root.dataset.uiThemeVariant, 'sylphie-06');
});
...
test('UI theme has its own settings window, separate from the floor setting', () => {
  ...
  assert.match(between, /id="uiThemeVariants"[^>]*role="radiogroup"/);
