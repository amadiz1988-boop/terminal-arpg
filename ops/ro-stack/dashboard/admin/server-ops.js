...
function renderCharacters() {
  ...
  const body = rows.map((c) => {
    ...
    const agentBusy = ca.status === 'activating' || ca.status === 'starting' || ca.status === 'recovering';
    const quarantined = c.ownershipState === 'QUARANTINED' && c.runtimeState === 'QUARANTINED';
    ...
    const detail = `<tr class="drawer"><td colspan="14"><div class="kv">
      ...
      <div class="row"><span>Admin 角色自主 / 掛機</span>
        ...
        <button class="action" data-support-debug="${c.charId}">以此角色身分偵錯</button>
        ${agentBusy ? `<span class="muted">${esc(ca.status === 'recovering' ? '安全恢復中…' : ca.status === 'activating' ? '啟用中…' : '啟動掛機中…')}</span>` : ''}</span></div>
      <div class="row"><span>Admin 執行狀態</span><span>${agentStatusBadge(ca)}</span></div>
      ${quarantined ? `<div class="row"><span>隔離恢復</span><span><button class="action" data-recover-char="${c.charId}"${agentBusy ? ' disabled' : ''}>安全恢復</button></span></div>` : ''}
...
function wireViewHandlers() {
  ...
  document.querySelectorAll('[data-support-debug]').forEach((b) => b.addEventListener('click', (ev) => {
    ev.stopPropagation();
    openSupportDebug(Number(b.dataset.supportDebug));
  }));
  document.querySelectorAll('[data-recover-char]').forEach((b) => b.addEventListener('click', (ev) => {
    ev.stopPropagation();
    confirmQuarantineRecovery(Number(b.dataset.recoverChar));
  }));
...
function openSupportDebug(charId) {
  const box = el('modal').querySelector('.box');
  box.innerHTML = `<h4>以此角色身分偵錯</h4>
    <label>原因 <input id="supportReason" maxlength="256" value="Synthetic Player scenario acceptance" /></label>
    <label>模式 <select id="supportMode"><option value="OBSERVE_ONLY">OBSERVE_ONLY</option><option value="PLAYER_ACTIONS">PLAYER_ACTIONS</option></select></label>
    <label>期限（分鐘） <input id="supportTtl" type="number" min="1" max="60" value="15" /></label>
    <div class="muted">支援 session 會顯示操作者、目標角色與剩餘時間，期限到期或撤銷後立即失效。</div>
    <div class="actions"><button class="action" data-cancel>取消</button><button class="action" data-confirm>建立支援 session</button></div>`;
  ...
  box.querySelector('[data-confirm]').addEventListener('click', async () => {
    const r = await api('/api/admin/support-sessions', { method: 'POST', body: JSON.stringify({
      effectiveCharId: charId,
      reason: box.querySelector('#supportReason').value,
      mode: box.querySelector('#supportMode').value,
      ttlMinutes: Number(box.querySelector('#supportTtl').value),
    }) });
    if (r.ok) {
      pushEvent('INFO', 'support-session', `#${charId} ${r.body?.supportSession?.supportSessionId ?? ''}`);
      window.location.href = '/';
    } else {
      pushEvent('ERROR', 'support-session', r.body?.error ?? `HTTP ${r.status}`);
...
function confirmQuarantineRecovery(charId) {
  const character = (state.characters ?? []).find((c) => c.charId === charId);
  if (!character || character.ownershipState !== 'QUARANTINED' ||
      character.runtimeState !== 'QUARANTINED') return;
  const box = el('modal').querySelector('.box');
  box.innerHTML = `<h4>確定將此角色安全恢復至待機狀態？</h4>
    <div class="row"><span>角色名稱</span><span>${esc(character.name)}</span></div>
    <div class="row"><span>Char ID</span><span>${charId}</span></div>
    <div class="row"><span>隔離原因</span><span>${esc(character.lastErrorCode || '未記錄')}</span></div>
    <div class="actions"><button class="action" data-cancel>取消</button><button class="action" data-confirm>安全恢復</button></div>`;
  el('modal').classList.add('on');
  box.querySelector('[data-cancel]').addEventListener('click', () => el('modal').classList.remove('on'));
  box.querySelector('[data-confirm]').addEventListener('click', async () => {
    el('modal').classList.remove('on');
    await runQuarantineRecovery(charId);
  }, { once: true });
}
async function runQuarantineRecovery(charId) {
  setCharAction(charId, { status: 'recovering', message: '安全恢復中…' });
  render();
  try {
    const submitted = await api(`/api/admin/characters/${charId}/quarantine-recovery`, { method: 'POST' });
    if (!submitted.ok || !submitted.body?.commandId) throw new Error(submitted.body?.error ?? `HTTP ${submitted.status}`);
    const commandId = submitted.body.commandId;
    setCharAction(charId, { status: 'recovering', message: '等待 Native 確認…', command: commandId });
    for (let attempt = 0; attempt < 40; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      const result = await api(`/api/admin/characters/${charId}/quarantine-recovery/${commandId}`);
      if (!result.ok || result.body?.state === 'FAILED')
        throw new Error(result.body?.reasonCode || result.body?.error || `HTTP ${result.status}`);
      if (result.body?.state === 'RECOVERED') {
        setCharAction(charId, { status: 'confirmed', message: '已安全恢復至待機', command: commandId });
        pushEvent('INFO', 'quarantine-recovery', `#${charId} RECOVERED ${commandId}`);
        await refreshCharacters();
        return;
      }
    }
    throw new Error('恢復尚未確認，請檢查權威狀態');
  } catch (error) {
    setCharAction(charId, { status: 'failed', message: error.message });
    pushEvent('ERROR', 'quarantine-recovery', `#${charId} ${error.message}`);
  }
  await refreshCharacters();
}
