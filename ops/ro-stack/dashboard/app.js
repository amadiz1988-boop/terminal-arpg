...
function renderAssassinQuest(runtime, character) {
  ...
  if (phase === 'IDLE') {
    ...
    description.textContent = '已抵達 Huey。按下接受後，rAthena 會再次驗證職業、Job Level、技能點與正式任務狀態。';
    ...
    description.textContent = 'rAthena 已完成 Assassin Job Change，任務狀態與 Dashboard 已同步。';
  } else {
    description.textContent = stage.status === 'FAILED'
      ? `本階段未通過。成績 ${Number(stage.score ?? 0)} / 10，可直接重試此階段。`
      : '正在等待 rAthena 回報正式任務狀態。';
  }
...
function renderRogueQuest(runtime, character) {
  ...
  if (phase === 'IDLE') {
    ...
    description.textContent = '資源已由 rAthena 扣除。聯絡人由伺服器決定並持久化，重新整理不會重抽。';
    ...
    description.textContent = '地下路線與正式 NPC 狀態已由 rAthena 確認。按下後執行最終 Job Change。';
    controls.append(jobQuestButton('完成轉職', commitRogueJobChange, 'primary'));
  } else if (phase === 'COMPLETED') {
    description.textContent = 'rAthena 已完成 Rogue Job Change，任務狀態與 Dashboard 已同步。';
  } else {
    description.textContent = stage.status === 'INVALID_INPUT' ? '密碼錯誤，可直接重試；前面進度已保留。' : stage.status === 'FAILED' ? `本階段未通過。成績 ${Number(stage.score ?? 0)}，可直接重試 Quiz。` : '正在等待 rAthena 回報正式任務狀態。';
  }
  root.replaceChildren(heading, description, controls);
...
function renderKnightQuest(runtime, character) {
  ...
  else if (phase === 'ITEM_REQUIRED') { const items = (stage.items ?? []).map((entry) => `${entry.name} × ${entry.amount}`).join('、'); description.textContent = `需交付：${items}。材料會由 rAthena 驗證並消耗。`; if (stage.status === 'INSUFFICIENT') { const missing = document.createElement('small'); missing.textContent = (stage.missingItems ?? []).map((entry) => `${entry.name} 尚缺 ${entry.missing}`).join('、'); controls.append(missing); } controls.append(jobQuestButton('重新檢查並提交', () => knightAction('SUBMIT_ITEMS'))); }
  ...
  else if (phase === 'ETHICS') { description.textContent = `依目前載入的 rAthena 程式判定作答。已嘗試 ${Number(knight.ethics?.attempts ?? 0)} 次。`; controls.append(choiceQuiz(stage, 'knight-ethics', 'SUBMIT_ETHICS', 10)); }
  ...
  else if (phase === 'COMPLETED') description.textContent = 'rAthena 已完成騎士 Job Change、任務完成與獎勵發放。';
  else description.textContent = stage.status === 'FAILED' ? '本階段未通過，可直接重試；先前 checkpoint 已保留。' : '正在等待 rAthena 回報正式任務狀態。';
  root.replaceChildren(heading, description, controls);
...
function renderCrusaderQuest(runtime, character) {
  ...
  else if (phase === 'MATERIAL_REQUIRED') { const items = (stage.items ?? []).map((entry) => `${entry.name} × ${entry.amount}`).join('、'); description.textContent = `需交付：${items}。材料由 rAthena 驗證並消耗。`; if (stage.status === 'INSUFFICIENT') { const missing = document.createElement('small'); missing.textContent = (stage.missingItems ?? []).map((entry) => `${entry.name} 尚缺 ${entry.missing}`).join('、'); controls.append(missing); } controls.append(jobQuestButton('重新檢查並提交', () => crusaderAction('SUBMIT_MATERIALS'))); }
  ...
  else if (phase === 'BATTLE_READY') { const rosaryReady = stage.equipmentRequirement?.ready === true; const water = Number(stage.holyWater?.current ?? 0); description.textContent = `${rosaryReady ? '念珠已裝備' : '需要裝備念珠'}，聖水 ${water} / 1。入場時由 rAthena 消耗 1 瓶聖水，四分鐘內完成三波不死系戰鬥。`; controls.append(jobQuestButton('檢查裝備、聖水與補給後開始', () => crusaderAction('START_BATTLE'), 'primary')); }
  ...
  else if (phase === 'FINAL_READY') { description.textContent = `所有考驗已由 rAthena 確認。獎勵為白色藥水 × ${Number(stage.reward?.amount ?? 6)}。`; controls.append(jobQuestButton('完成轉職', commitCrusaderJobChange, 'primary')); }
  else if (phase === 'COMPLETED') description.textContent = 'rAthena 已完成十字軍 Job Change、任務完成與獎勵發放。';
  else description.textContent = stage.status === 'EQUIPMENT_REQUIRED' ? '需要裝備念珠才能開始試煉。' : stage.type === 'SUPPLY' ? '缺少聖水或戰鬥補給，正在依安全中斷規則處理。' : ['FAILED','INSUFFICIENT','READY_TO_RETRY'].includes(stage.status) ? '本階段可直接重試，既有 checkpoint 已保留。' : '正在等待 rAthena 回報正式任務狀態。';
  root.replaceChildren(heading, description, controls);
...
document.addEventListener('click', (event) => {
  ...
  if (button.matches('[data-stat]')) {
    window.RoOriginalUiKit?.RoUiSound.play('ui.status.increase');
    return;
  }
