...
點城名只選擇並開啟右側詳情，不立即傳送。城鎮面板依序顯示：城名、`【傳送至<主城>】`、符合實際在場條件時的 `【設為儲存主城】`，接著是 Map ID、類型「主城」、費用「免費」、目前傳送冷卻、目前儲存主城與相關資訊。點傳送後顯示「是否傳送至普隆德拉？」及「傳送費用：免費」，按鈕為「取消」、「確定傳送」。權威到達成功後才播放經來源驗證的 RO 傳送音、關閉確認、更新當前地圖並顯示非阻斷提示「已抵達 普隆德拉」；不再顯示第二個確認視窗。失敗不播成功音。
