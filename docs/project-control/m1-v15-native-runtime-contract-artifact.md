...
- `OPTIONAL_FIELD_BOUNDARY`: each distance field is optional in the registry and all three fields can be omitted. Native `parse_farm_payload` at `src/map/persistent_agent.cpp:1225-1240` requires `attackDistance` and `attackMaxDistance` together when either is supplied, plus an explicit combat profile. The Web controller supplies the pair together. Standalone-distance Native acceptance is not proven and cannot be reported as PASS for the V15 individual-field test rows without a clarifying contract test or bounded Native correction.
