...
| H05 Search teleport | `src/AI/CoreLogic.pm` `processAutoTeleport`; `teleportAuto_search=0` | 0; YES | Search condition selects teleport with cooldown and map safety | `try_hunt_relocation` after no target, item allowlist | PARTIAL | Compare exact search condition, bounded retries and item fallback; PA intent to rAthena item authority |
| H06 Danger / escape teleport | `src/AI/CoreLogic.pm` `processAutoTeleport`, `src/Misc.pm` damage callbacks; `teleportAuto_hp=10`, `teleportAuto_deadly=1`, `teleportAuto_maxDmg=500` | 10 / 1 / 500; YES | Distinct low-HP, lethal and damage triggers request teleport | Native survival handles HP; no proven emergency teleport bridge | MISSING | Add separately typed bounded emergency policy after predicate proof; existing PA interrupt and native item seam |
...
| H14 No-target random walk | `src/AI/CoreLogic.pm` `processRandomWalk`; `route_randomWalk=1`, `route_randomWalk_inTown=0` | 1 / 0; YES | Idle/search may choose bounded random movement | Native no-target scan and optional hunt relocation | PARTIAL | Separate search, idle roam and farm containment; existing farm tick |
