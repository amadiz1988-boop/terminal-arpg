...
| H15 Lock-map return | `src/AI/CoreLogic.pm` `processLockMap`; `lockMap` unset | unset; YES | Selected lock map queues route back when off target | PA persists farm target and uses return navigation after supply/death | PARTIAL | Compare return trigger, interruption and arrival; existing PA journey |
| H16 Missing portal repair | `src/AI/CoreLogic.pm` `processReAddMissingPortals`; `src/Task/MapRoute.pm` `iterate`; `route_removeMissingPortals_NPC=1`, `route_removeMissingPortals=0`, `route_reAddMissingPortals=1` | 1 / 0 / 1; YES | Missing or failed portal may be pruned, guessed or re-added before replan | GI world graph and bounded route failure exist; adoption audit open | PARTIAL | One-engine missing-edge/replan parity fixture; no per-map workaround |
| H17 Portal-avoidance teleport | `src/AI/CoreLogic.pm:3719-3731` `processAutoTeleport`; `teleportAuto_portal=0` | off; YES | Optional non-city, safe, lock-map and service-queue guards before avoiding visible portal | GI farm chase excludes exit portal cells; no optional portal-avoid teleport | NOT_IMPLEMENTED | Later advanced roaming/teleport stage; HOLD, retain native portal containment |
...
| C04 Offensive skill slot | `control/config.txt` `attackSkillSlot` block; `src/AI/Attack.pm` `main`; `src/Misc.pm:5660-5675` `checkSelfCondition` | Empty skill name, maxAttempts/maxUses 0; YES | First matching slot checks self and monster predicates, learned skill and SP cost, then cast parameters and attempts | Ordered numeric HP/SP/timeout subset reaches Native; learned/SP-unavailable rows skip; rAthena validates cast | PARTIAL | M1 target/ground predicates and authoritative multi-skill effect proof; existing rAthena skill primitive |
...
| C17 Attack combo | `control/config.txt:633-645` `attackComboSlot`; `src/AI/Attack.pm:682-720` `main` | no active combo row; YES | Previous skill, target, max uses and combo delay gate the next action | Native single skill path, no combo scheduler | NOT_IMPLEMENTED | M1 Stage 3A after separate promotion; HOLD |
| C18 Reactive monster skill | `control/config.txt:681-689` `monsterSkill`; `src/AI/CoreLogic.pm:3182-3214` `processMonsterSkillUse` | no active row; YES | Self and monster predicates, uses and target determine cast | Native one skill attack path; no reactive monster-skill scheduler | NOT_IMPLEMENTED | M1 Stage 3A/B after separate promotion; HOLD |
| C19 Target priority / party assist | `src/AI/CoreLogic.pm:3390-3449` `processAutoAttack`; `attackAuto_party=1`, `attackLooters=0` | 1 / 0; YES | Skill-cancel, looter, aggressive, party, dropped, then clean target priority with LOS | Native target scan/blocked set; full priority and KS equivalence unproven | PARTIAL | M1 target/retarget fixture matrix; existing Native target search |
...
| R02 SP sit | `src/AI/CoreLogic.pm:2909-2962` `processSitAuto`; `control/config.txt:292-293` `sitAuto_sp_lower/upper` | 0/0; YES | Disabled by default, opt-in independent of action skill SP | Native source 0/0, Production still 20/40 | PARTIAL | Controlled deploy plus route/combat opt-in proof; PA recovery |
| R03 Sit / stand gating | `src/AI/CoreLogic.pm:2909-2962` `processSitAuto`; `control/config.txt:294-296` `sitAuto_over_50/idle` | 0/1; YES | Basic skill, queue action, weight and aggression gate sit; safe upper stand | Native recovery sit lacks full gate equivalence | PARTIAL | Port explicit decision gate and bounded safe-resume tests; PA recovery |
| R04 Recovery item / potion | `control/config.txt` `useSelf_item` empty; `src/AI/CoreLogic.pm` `processAutoItemUse` | no active row; YES | Per-row self condition, item presence and timeout select use | Character HP-percent item rows reach Native `pc_useitem`; legacy global allowlist remains | PARTIAL | Authoritative HP/count effect, condition overlap and Browser proof; rAthena use authority |
...
| R12 Shop sell | `src/AI/CoreLogic.pm` `processAutoSell`; `sellAuto=0` | off; YES | Optional NPC sell route after weight/service decision | Native `service_shop_sell` primitive exists; autonomous sell scheduling is absent | NOT_IMPLEMENTED | OpenKore Exit C9 / later supply service gate; reuse existing NPC/shop authority |
| R13 Storage | `src/AI/CoreLogic.pm` `processAutoStorage`; `storageAuto=0` | off; YES | Optional NPC storage, capacity, retry and next service | Native `service_storage_deposit/withdraw` primitives exist; GI Global AutoStore explicitly false | GI_EXPLICIT_OVERRIDE | Keep `FIXED_POLICY.loot.autoStore=false`; manual storage uses existing primitives |
...
| R23 Per-item pickup policy | `control/pickupitems.txt` `all 1`; `src/Misc.pm:3689` `pickupitems`; `src/AI/CoreLogic.pm:3468` `processItemsTake` | pickup all; YES | Per-item -1/0/1/2 controls drop, skip, pickup and priority | GI Global AutoLoot is on; item-rule config exists, full Native per-item filtering unproven | PARTIAL | Preserve Global AutoLoot and test explicit item exceptions; rAthena inventory authority |
