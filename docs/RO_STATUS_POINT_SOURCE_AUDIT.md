...
本作技能分類依 rAthena 技能資料與 OpenKore 戰鬥事件。普通攻擊使用 RO 物理攻擊與 AGI 間隔，魔法使用 RO MATK 與 Renewal 可變詠唱，弓箭使用 RO 遠程攻擊。INT 依 RO 規則影響最大 SP。配點與重置會立即重新解析戰鬥能力，進行中的地圖會在下一個戰鬥步更新生命與 SP 上限。
