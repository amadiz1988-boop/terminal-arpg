...
- 超級初心者完成初心者 Job Lv.10 與基本技能 Lv.9 後，學院導師會依 Base Lv.送往 `prt_fild08`、`moc_fild11` 或 `pay_dun00` 練至 Base Lv.45。玩家可雙擊任務日誌的「一轉結業」恢復流程；未達標時返回推薦練功地，達標後直接傳回學院。
- 達標後由 OpenKore 暫停戰鬥並快速引導至伊斯魯得島 `Criatura Academy`。
- `iz_ac01 (60,67)` 的結業導師再次檢查角色職業、Job 等級、基本技能與創角志願。
...
- 線上角色 `賴清德` 的永久志願確實為 `gunslinger`，舊版學院 NPC 因只辨識六個標準一轉而顯示錯誤訊息。
- 學院 NPC 與 OpenKore 路由已補齊超級初心者、跆拳、神槍手、忍者四個分支。
- 超級初心者新增 Base Lv.45 前往返練功流程，並保留 rAthena Job Master 的等級門檻。
- 神槍手與忍者的快速結業會分別補至起始武器所需的 Base Lv.10、12，再裝備武器與彈藥；既有未完成裝備的角色會由受限修復指令補正。
- 以指定 NPC 檔案熱更新完成線上套用，login、character、map 三項服務均未重新啟動。實測角色 `賴清德` 已成為 Job ID 24 神槍手，Base Lv.10、Job Lv.1，並裝備 `Six Shooter [2]` 與 500 發子彈後返回 `prt_fild08`。
